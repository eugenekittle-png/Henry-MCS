...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const readable = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          logAction({ username: session?.username ?? null, action: "Compare", clientNumber, matterNumber, details: contextDetails, tokensInput, tokensOutput, success, ipAddress: ip });
        }
      },
    });
    ...
    logAction({ username: session?.username ?? null, action: "Compare", details: { error: message }, success: false, ipAddress: ip });
