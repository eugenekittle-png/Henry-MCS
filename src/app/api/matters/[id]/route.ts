...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await logAction({ username: session?.username ?? null, action: "matter_update", details: { matterId, matterNumber: matter_number, description }, success: true });
    return Response.json(updated);
  } catch (err) {
    const message = err instanceof Error ? err.message : "Failed to update matter";
    const status = message.includes("UNIQUE") ? 409 : 500;
    await logAction({ username: session?.username ?? null, action: "matter_update", details: { error: message }, success: false });
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await logAction({ username: session?.username ?? null, action: "matter_delete", details: { matterId, matterNumber: existing.matter_number, description: existing.description }, success: true });
